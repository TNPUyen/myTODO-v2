...
func (bl *InvitationBusinessLogic) CreateInvitation(invitation *models.Invitation) error {
	alrInv := bl.server.Db.Collection("invitations").FindOne(context.Background(), bson.D{primitive.E{Key: "receiver_id", Value: invitation.Receiver_id}, primitive.E{Key: "project_id", Value: invitation.Project_id}})
	if alrInv.Err() == nil {
